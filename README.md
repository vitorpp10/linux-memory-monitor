...
In main(), we simulate a load:
    ...
    Initial check: print baseline usage.
    Allocation: we fire up new int[1024 * 1024 * 10]. This puts a massive array on the heap.
    Dirtying pages: we loop through and fill the array. This is crucial—Linux is lazy. It won't actually map physical RAM until you write to it.
    Cleanup: we call delete[] to free the memory.
